...
namespace Order.Application.Orders.Queries.GetOrdersByCustomerId
{
        ...
        : IQueryHandler<GetOrderByCustomerIdQuery, List<GetOrderByCustomerIdResponse>>
    {
        ...
        {
            var orders = await _orderRepository.GetAllAsync(cancellationToken, x => x.Id == request.CustomerId, o => o.Include(x => x.Products));

            var data = new List<GetOrderByCustomerIdResponse>();
            foreach (var order in orders)
            {
                data.Add(new GetOrderByCustomerIdResponse(order.Id, order.CreatedAt, order.UpdatedAt, order.CustomerId, order.Quantity, order.Price, order.Status, order.Products));
            }
